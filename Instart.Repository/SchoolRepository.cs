    ...
    {     
        ...
        public PageModel<School> GetListAsync(int pageIndex, int pageSize, string name = null) {
            using (var conn = DapperFactory.GetConnection()) {
                ...
                string sql = string.Format(@"select * from ( select *, ROW_NUMBER() over (order by name collate Chinese_PRC_CI_AS) as RowNumber from [School] {0} ) as b  
        ...
        public bool InsertAsync(School model) {
            using (var conn = DapperFactory.GetConnection()) {
                var fields = model.ToFields(removeFields: new List<string> { "Id", "IsRecommend", "IsHot", "IsSelected" });
